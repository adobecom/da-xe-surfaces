...
const FONT_PRESET_KEYS = new Set([
...
/**
 * Apply font preset and/or style in one call. Preset adds class; non-preset value is parsed as CSS.
 * @param {string} value - Preset key (e.g. "heading-xl") or CSS declarations (e.g. "font-weight: 700")
 * @param {Element} element - Element to apply to.
 */
export function decorateFont(value, element) {
  if (!value?.trim() || !element) return;
  const raw = value.trim();
  const presetKey = raw.toLowerCase().replace(/\s+/g, '-');
  if (FONT_PRESET_KEYS.has(presetKey)) {
    element.classList.add(presetKey);
    return;
  }
  if (!element.style) return;
  const declarations = raw.split(/\s*[,;]\s*/).map((s) => s.trim()).filter(Boolean);
  declarations.forEach((decl) => {
    ...
    element.style[kebabToCamel(prop)] = normalizeCssValue(val);
  });
}
